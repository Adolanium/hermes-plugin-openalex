...
OPENALEX_RESOLVE = {
    ...
    "description": (
        "FREE, turns a name into an OpenAlex id you can filter on.\n\n"
        "It keeps working even when the daily budget is gone.\n\n"
...
OPENALEX_GET = {
    "name": "openalex_get",
    "description": (
        "FREE, fetches records by id, DOI, ORCID, ROR, ISSN or PMID.\n\n"
        "It keeps working at zero budget.\n\n"
...
OPENALEX_ACCOUNT = {
    ...
    "description": (
        "FREE, reports OpenAlex budget, prices and session spend.\n\n"
        "The account's daily allowance and what is left of it, plus this "
        "session's spend broken down by call type.\n\n"
...
OPENALEX_FIELDS = {
    ...
    "description": (
        "FREE and local, lists valid filter and select field names.\n\n"
        "Covers every entity plus the vocabularies (work types, languages, "
        "SDGs, institution types). Served from disk, so it makes no API call "
        "and cannot fail on budget.\n\n"
...
OPENALEX_COUNT = {
    "name": "openalex_count",
    "description": (
        "Counts and groups records, ten times cheaper than a search.\n\n"
        "COSTS $0.0001 against $0.001 for openalex_search, and returns "
...
OPENALEX_RELATED = {
    ...
    "description": (
        "Walks the citation graph from one work, at cheap list price.\n\n"
        "COSTS $0.0001.\n\n"
        ...
        "For the full method, load the openalex:lit-review skill. It covers "
        "the cheap ordering (anchor, size, shape, traverse, retrieve) and how "
        "to read the data honestly, including citation counts favouring older "
        "work and uneven coverage outside Crossref-indexed journals."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
OPENALEX_SEARCH = {
    ...
    "description": (
        "Returns records at ten times the price of a count.\n\n"
        ...
        "Two skills are available and neither is loaded unless you ask for it: "
        "openalex:query-syntax for the filter language and the cost model, and "
        "openalex:lit-review for the cheap way to map a literature.\n\n"
...
OPENALEX_CLASSIFY = {
    ...
    "description": (
        "The most expensive call, classifies text into topics.\n\n"
...
OPENALEX_HARVEST = {
    ...
    "description": (
        "Cursor-pages a large result set, billed once per page.\n\n"
        "Pages up to a declared maximum number of records.\n\n"
...
OPENALEX_FULLTEXT = {
    "name": "openalex_fulltext",
    "description": (
        "Reports free if full text exists, downloads on confirm.\n\n"
        "Call it WITHOUT confirm first. That costs nothing and tells you "
        "whether OpenAlex holds the full text, in which formats, and what "
        "downloading would cost. Nothing is fetched and nothing is spent.\n\n"
        "Call it again with confirm=true ONLY when the user has actually asked "
        "for the full text of the paper. The download COSTS $0.01, a hundred "
        "times a list call, so ten of them exhaust an entire anonymous day. Do "
        "not download speculatively, and do not download a paper just to "
        "summarise something the abstract already covers.\n\n"
        "OpenAlex holds content for roughly 54 million works, a fraction of "
        "the index, so absence is normal. When there is no content, or only a "
        "PDF, the response hands back a URL instead and spends nothing. "
        "Hermes can fetch that URL with its own web tools for free."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "id": {
                "type": "string",
                "description": "Work identifier: OpenAlex id, DOI, PMID or the URL form of either.",
            },
            "confirm": {
                "type": "boolean",
                "description": (
                    "false or omitted checks availability for free. true "
                    "downloads and costs $0.01. Only set it when the user "
                    "asked for the full text."
                ),
                "default": False,
            },
            "format": {
                ...
                "enum": ["text", "pdf"],
                "description": (
                    "'text' extracts readable prose from the TEI, which is "
                    "what a model can use. 'pdf' returns the link only."
                ),
                "default": "text",
            },
        },

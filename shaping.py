...
def tei_to_text(xml: str, limit: int = 0) -> str:
    """Flatten GROBID TEI XML into readable prose.

    OpenAlex serves full text as TEI produced by GROBID. Stripping the markup
    roughly halves it (48,955 characters of TEI became 26,059 of text on a
    measured paper), and a model reading prose does far better than one
    reading angle brackets.

    Deliberately simple. A real TEI parse would preserve section structure,
    but that means a dependency, and the value here is the words.
    """
    import html
    import re
    ...
    if not xml:
        return ""
    # Drop the parts carrying no prose before stripping the rest.
    body = re.sub(r"(?is)<(teiHeader|listBibl|figure|formula)\b.*?</\1>", " ", xml)
